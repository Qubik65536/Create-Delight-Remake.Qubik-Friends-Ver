...
function main_track_states.idle.transition(this, context, input)
    ...
    -- 新增：当玩家按下切换射击模式键时，播放切换模式的动画（在 SWITCH_MODE_TRACK 上）
    if (input == INPUT_FIRE_SELECT) then
        -- 根据当前模式决定播放哪一段切换动画
        if (context:getFireMode() == SEMI) then
            context:runAnimation("select_auto", context:getTrack(STATIC_TRACK_LINE, SWITCH_MODE_TRACK), false, PLAY_ONCE_STOP, 0)
        elseif (context:getFireMode() == AUTO) then
            context:runAnimation("select_semi", context:getTrack(STATIC_TRACK_LINE, SWITCH_MODE_TRACK), false, PLAY_ONCE_STOP, 0)
        else
            -- 如果还有其他模式，可以在这里扩展对应动画
        end
        -- 保持在闲置态
        return this.main_track_states.idle
    end
